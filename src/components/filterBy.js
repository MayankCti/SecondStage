...
function FilterBy(props) {
  ...
  const getAllBrand = () => {
    setIsLoader(true);
    axios({
      url: configJSON.baseUrl + configJSON.getAllBrand,
      method: "get",
    })
      .then((res) => {
        setIsLoader(false);
  ...
  useEffect(() => {
    if (pathname == "/bikinis") {
      setCategory([...category, "Bikini"]);
    }
    if (pathname == "/figure") {
      setCategory([...category, "Figure"]);
    }
    if (pathname == "/swimsuit") {
      setCategory([...category, "Swimsuit"]);
    }
    if (pathname == "/wbff") {
      setCategory([...category, "FMG_WBFF"]);
    }
    if (pathname == "/themewear") {
      setCategory([...category, "Themewear"]);
    }
    if (pathname == "/accessories") {
      setCategory([...category, "Accessories_Shoes"]);
    }
  }, []);
  ...
  const handleStyleBottom = (val) => {
    if (styleBottom.includes(val)) {
      setStyleBottom((styleBottom) =>
        styleBottom.filter((item) => item != val)
      );
    } else {
  ...
  const handleSizeStandard = (val) => {
    if (sizeStandard.includes(val)) {
      setSizeStandard((sizeStandard) =>
        sizeStandard.filter((item) => item != val)
      );
    } else {
  ...
  const handleBilingLevel = (val) => {
    if (bilingLevel.includes(val)) {
      setBilingLevel((bilingLevel) =>
        bilingLevel.filter((item) => item != val)
      );
    } else {
  ...
  const handleBilingCondition = (val) => {
    if (bilingCondition.includes(val)) {
      setBilingCondition((bilingCondition) =>
        bilingCondition.filter((item) => item != val)
      );
    } else {
  ...
  const filter = () => {
    ...
    const data = {
      ...
      price_sale_lend_price_min_rent:
        rentPrice?.length != 0 ? [rentPrice[0]] : [],
      price_sale_lend_price_max_rent:
        rentPrice?.length != 0 ? [rentPrice[1]] : [],
                    ...
                    >
                      ...
                      <div className="price-range__info d-flex align-items-center mt-2">
                        <div className="me-auto">
                          <span className="text-secondary">Min Price: </span>
                          <span className="price-range__min">
                            ${rentPrice?.length != 0 ? rentPrice[0] : 0}
                          </span>
                        </div>
                        <div>
                          <span className="text-secondary">Max Price: </span>
                          <span className="price-range__max">
                            ${rentPrice?.length != 0 ? rentPrice[1] : 0}
                          </span>
                        </div>
                      </div>
                            ...
                            <li className="list-item d-flex align-items-center">
                              <label className="cyberpunk-checkbox-label">
                                <input
                                  type="checkbox"
                                  className="cyberpunk-checkbox"
                                  onChange={() => handleSizeStandard(item)}
                                />
                              </label>
                              {item}
                            ...
                            <li className="list-item d-flex align-items-center">
                              <label className="cyberpunk-checkbox-label">
                                <input
                                  type="checkbox"
                                  className="cyberpunk-checkbox"
                                  onChange={() => handleSizeBottom(item)}
                                />
                              </label>
                              {item}
                              {/* <a  className="menu-link py-1"><h5>Bikini/Fitness</h5></a> */}
                    ...
                    {filterContent?.productColor?.map((item) => (
                      <>
                        <a
                          ...
                          title={item}
                          className={`swatch-color js-filter ${color.includes(item) ? 'swatch_active' : ''}`}
                          style={{ color: `${item}` }}
                            ...
                            <li className="list-item d-flex align-items-center">
                              <label className="cyberpunk-checkbox-label">
                                <input
                                  type="checkbox"
                                  className="cyberpunk-checkbox"
                                  onChange={() => handleBilingLevel(item)}
                                />
                              </label>
                              {item}
                              {/* <a  className="menu-link py-1"><h5>Bikini/Fitness</h5></a> */}
                            ...
                            <li className="list-item d-flex align-items-center">
                              <label className="cyberpunk-checkbox-label">
                                <input
                                  type="checkbox"
                                  className="cyberpunk-checkbox"
                                  onChange={() => handleBilingType(item)}
                                />
                              </label>
                              {item}
                              {/* <a  className="menu-link py-1"><h5>Bikini/Fitness</h5></a> */}
                      ...
                      <li className="list-item d-flex align-items-center">
                        <label className="cyberpunk-checkbox-label">
                          <input
                            type="checkbox"
                            className="cyberpunk-checkbox"
                            onChange={() => handleBilingCondition(item)}
                          />
                        </label>
                        {item}
                        {/* <a  className="menu-link py-1"><h5>Bikini/Fitness</h5></a> */}
