...
function Register_seller() {
  ...
  const [isSizeTop, setIsSizeTop] = useState(false);
  const [isSizeBottom, setIsSizeBottom] = useState(false);
  const [isSizeStandard, setIsSizeStandard] = useState(false);
  const [isStyleTop, setIsStyleTop] = useState(false);
  const [isStyleBottom, setIsStyleBottom] = useState(false);
  const [isBlingType, setIsBlingType] = useState(false);
  const [isBlingLevel, setIsBlingLevel] = useState(false);
  const [isBlingCondition, setIsBlingCondition] = useState(false);
  const [isPriceBuy, setIsPriceBuy] = useState(false);
  const [isPadding, setIsPadding] = useState(false);
  const [isLengthRental, setIsLengthRental] = useState(false);
  ...
  const [priceBuyOption, setPriceBuyOption] = useState()
  const [blingConditionOption, setBlingConditionOption] = useState()
  const [blingLevelOption, setBlingLevelOption] = useState()
    ...
    <>
      <svg className="d-none">
        ...
        <symbol id="icon_close" viewBox="0 0 12 12">
          <path
            d="M0.311322 10.6261L10.9374 0L12 1.06261L1.37393 11.6887L0.311322 10.6261Z"
            fill="currentColor"
          />
          <path
            d="M1.06261 0.106781L11.6887 10.7329L10.6261 11.7955L0 1.16939L1.06261 0.106781Z"
            fill="currentColor"
          />
        </symbol>
                ...
                >
                  <ul className="nav nav-tabs mb-3 text-uppercase ct_active_btn1 justify-content-center gap-3 mb-5" id="collections-tab" role="tablist">
                    <li className="nav-item" role="presentation">
                      <a className={`nav-link nav-link_underscore ct_sell_btn ct_btn_large  ${product == 'product' && 'ct_active'}`} onClick={() => setProduct("product")}>Product</a>
                    </li>
                    <li className="nav-item" role="presentation">
                      <a className={` nav-link nav-link_underscore ct_sell_btn  ct_btn_large w-100 ${product == 'featured' && 'ct_active'}`} onClick={() => setProduct("featured")}>Featured Product</a>
                    </li>
